...
namespace Botan {
    ...
    void GeneralName::decode_from(class BER_Decoder &ber) {
        ...
        if (tag == 1 || tag == 2 || tag == 6) {
            ...
            if (obj.value.size() == 8) {
                const std::vector<byte> ip(obj.value.begin(),
                        obj.value.begin() + 4);
                const std::vector<byte> net(
                        obj.value.begin() + 4, obj.value.end());
                m_type = "IP";
                m_name = ipv4_to_string(load_be<u32bit>(ip.data(), 0)) + "/" +
                         ipv4_to_string(load_be<u32bit>(net.data(), 0));
            }
    ...
    bool GeneralName::matches_dns(const std::string &nam) const {
        ...
        else // name.size() < nam.size()
        {
            ...
            return constr ==
                   nam.substr(nam.size() - constr.size(), constr.size());
        }
    }
